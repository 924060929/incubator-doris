...
public class ResolveHaving implements AnalysisRuleFactory {
    ...
    public List<Rule> buildRules() {
        return ImmutableList.of(
            RuleType.RESOLVE_HAVING_REPEAT.build(
                logicalHaving(logicalAggregate(logicalRepeat())).thenApply(ctx -> {
                    LogicalHaving<LogicalAggregate<LogicalRepeat<GroupPlan>>> having = ctx.root;
    ...
    private Plan createGroupByPlan(
            LogicalHaving<LogicalAggregate<LogicalRepeat<GroupPlan>>> having, Resolver resolver) {
        LogicalAggregate<LogicalRepeat<GroupPlan>> aggregate = having.child();
        LogicalRepeat<GroupPlan> groupBy = aggregate.child();
        ...
        LogicalAggregate<LogicalRepeat<Plan>> newAgg = new LogicalAggregate<>(aggregate.getGroupByExpressions(),
                ...
                groupBy.replace(groupBy.getGroupingSets(),
                        ...
                        groupBy.hasChangedOutput(), groupBy.isNormalized()));
        LogicalFilter<LogicalAggregate<LogicalRepeat<Plan>>> filter =
