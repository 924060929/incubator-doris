...
import org.apache.doris.nereids.worker.BackendWorkerManager;
...
public class AssignedJobBuilder {
    ...
    private static ListMultimap<ExchangeNode, AssignedJob> getInputAssignedJobs(
        ...
        for (Entry<ExchangeNode, UnassignedJob> exchangeNodeToChildJob
                : unassignedJob.getExchangeToChildJob().entrySet()) {
            ExchangeNode exchangeNode = exchangeNodeToChildJob.getKey();
            UnassignedJob childJob = exchangeNodeToChildJob.getValue();
