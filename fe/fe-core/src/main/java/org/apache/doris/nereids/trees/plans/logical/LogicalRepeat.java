...
public abstract class LogicalRepeat<CHILD_TYPE extends Plan>
    ...
    public LogicalRepeat(
        ...
        this.groupingSetIdSlots = ImmutableList.of();
    ...
    public Set<VirtualSlotReference> getVirtualSlotRefs() {
        return getVirtualGroupByExpressions().stream()
    ...
    public int hashCode() {
        return Objects.hash(groupByExpressions, outputExpressions,
                groupingSetIdSlots, groupingSets, isNormalized);
    }
    ...
    public List<GroupingSetIdSlot> genGroupingSetIdSlots(
            List<List<Expression>> groupingSets,
            List<Expression> nonVirtualGroupByExpressions,
            List<NamedExpression> outputExpressions) {
        return new ImmutableList.Builder<GroupingSetIdSlot>()
                .add(genExtraVirtualGroupingSetIdSlot(groupingSets, nonVirtualGroupByExpressions))
                .addAll(genGroupingFuncGroupingSetIdSlot(
                        groupingSets, nonVirtualGroupByExpressions, outputExpressions))
                .build();
    }

    public GroupingSetIdSlot genExtraVirtualGroupingSetIdSlot(
            List<List<Expression>> groupingSets, List<Expression> nonVirtualGroupByExpressions) {
        return new GroupingSetIdSlot(new HashSet<>(nonVirtualGroupByExpressions),
                genShouldBeErasedToNull(groupingSets, nonVirtualGroupByExpressions));
    }

    private List<GroupingSetIdSlot> genGroupingFuncGroupingSetIdSlot(
            List<List<Expression>> groupingSets, List<Expression> nonVirtualGroupByExpressions,
            List<NamedExpression> outputExpressions) {
        List<GroupingSetIdSlot> groupingSetIdSlots = new ArrayList<>();
        outputExpressions.stream()
                .filter(Alias.class::isInstance)
                .map(Alias.class::cast)
                .filter(e -> e.child() instanceof GroupingScalarFunction)
                .forEach(e ->
                    groupingSetIdSlots.add(new GroupingSetIdSlot(
                            new HashSet<>(nonVirtualGroupByExpressions),
                            genShouldBeErasedToNull((GroupingScalarFunction) e.child(), groupingSets)))
                );
        return groupingSetIdSlots;
    }
     ...
     */
    public List<List<Boolean>> genShouldBeErasedToNull(
